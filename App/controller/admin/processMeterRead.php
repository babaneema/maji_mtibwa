...
// $_POST['token'] == $_SESSION['_addReadMeter_token']
if (true) {
    // Getform form data
    ...
    $unitdata = $database->query(
        "SELECT unit_id, unit_unique, unit_branch, unit_price, unit_reg_date 
        FROM unit 
        WHERE unit_branch = $cust_branch
    ...
    $bill = new \App\Model\Bill(customer: $cust_id, meter: $cust_meter_id, unit: $unitdata["unit_id"], used: $new_unit, cost: $new_bill_cost);
    ...
    if(!$save){
        ...
        header('Location: ../../view/admin/readMeter.php?id='.$meter_unique);
        exit;
    }
    ...
    header('Location: ../../view/admin/viewSingleMeterStatus.php?id='.$meter_unique);
